...
void VideoTimingCaculator::calculateCvt(uint32_t horiz_pixels, uint32_t vert_pixels,uint32_t refresh_rate,
...
int VideoTimingCaculator::getMaxVFreq(  uint32_t horiz_pixels,uint32_t vert_pixels,uint32_t pix_clock,
                                        uint32_t bpc,double color_fmt_multiplier,string reduced_blanking,
                                        bool margins,bool interlaced,bool video_optimized){
    VideoTimingCaculator caculator;
    uint32_t rate=0;
    timing_t video_timing;
    while(rate<MAX_FREQ){
        caculator.calculateCvt(horiz_pixels,vert_pixels,++rate,bpc,color_fmt_multiplier,
                                reduced_blanking,margins,interlaced,video_optimized);
        video_timing=caculator.getCvtTiming();
        if(video_timing.pix_clock>pix_clock){
            NLOGI("%s %d\n",__FUNCTION__,rate);
            return rate-1;
        }
    }
    NLOGE("%s to max freq %d exit!\n",__FUNCTION__,MAX_FREQ);
    return V_ERROR;
}

int VideoTimingCaculator::getMinHPixs( uint32_t vert_pixels,uint32_t refresh_rate,uint32_t pix_clock,
                                        uint32_t bpc,double color_fmt_multiplier,string reduced_blanking,
                                        bool margins,bool interlaced,bool video_optimized){
